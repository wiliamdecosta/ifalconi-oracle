<?php
//Include Common Files @1-B91DA71D
define("RelativePath", "..");
define("PathToCurrentPage", "/pay_param/");
define("FileName", "P_AREA.php");
...
//Class_Initialize Event @2-E20C49F8
    function clsGridGRID($RelativePath, & $Parent)
    {
        ...
        $this->DLink->Page = "P_AREA.php";
        $this->ADLink = & new clsControl(ccsLink, "ADLink", "ADLink", ccsText, "", CCGetRequestParam("ADLink", ccsGet, NULL), $this);
        $this->ADLink->HTML = true;
        $this->ADLink->Page = "P_AREA.php";
        ...
        $this->GRID_Insert->Page = "P_AREA.php";
    }
    ...
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->DLink->Parameters = CCAddParam($this->DLink->Parameters, "p_area_id", $this->DataSource->f("p_area_id"));
                $this->ADLink->Parameters = CCGetQueryString("QueryString", array("FLAG", "TAMBAH", "ccsForm"));
                $this->ADLink->Parameters = CCAddParam($this->ADLink->Parameters, "p_area_id", $this->DataSource->f("p_area_id"));
        ...
        $this->GRID_Insert->Parameters = CCGetQueryString("QueryString", array("p_area_id", "ccsForm"));
...
//DataSource Variables @2-101B7449
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-6F1F10B1
    ...
    function Operation()
    {
        ...
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "P_AREA.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
//Class_Initialize Event @61-BA68F84D
...
//Initialize Method @61-38246EA7
    function Initialize()
    {
        ...
        $this->DataSource->Parameters["urlp_area_id"] = CCGetFromGet("p_area_id", NULL);
    }
//End Initialize Method

//Validate Method @61-9DD815C8
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
...
//CheckErrors Method @61-A867BA7A
    function CheckErrors()
    {
        $errors = false;
...
//Operation Method @61-4DB3C42C
    function Operation()
    {
        ...
        $Redirect = "P_AREA.php" . "?" . CCGetQueryString("QueryString", array("ccsForm"));
        if($this->PressedButton == "Button_Delete") {
            $Redirect = "P_AREA.php" . "?" . CCGetQueryString("QueryString", array("ccsForm", "FLAG", "TAMBAH", "s_keyword", "p_area_id"));
            if(!CCGetEvent($this->Button_Delete->CCSEvents, "OnClick", $this->Button_Delete) || !$this->DeleteRow()) {
                $Redirect = "";
            }
        } else if($this->PressedButton == "Button_Cancel") {
            $Redirect = "P_AREA.php" . "?" . CCGetQueryString("QueryString", array("ccsForm", "FLAG", "TAMBAH"));
            if(!CCGetEvent($this->Button_Cancel->CCSEvents, "OnClick", $this->Button_Cancel)) {
                $Redirect = "";
            }
        } else if($this->Validate()) {
            if($this->PressedButton == "Button_Insert") {
                $Redirect = "P_AREA.php" . "?" . CCGetQueryString("QueryString", array("ccsForm", "FLAG", "TAMBAH", "s_keyword"));
...
//DataSource Variables @61-B919F9DE
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @61-CAC36F88
...
//Update Method @61-9C8B5492
    function Update()
    {
        ...
        $this->cp["CREATION_DATE"] = new clsSQLParameter("ctrlCREATION_DATE", ccsDate, array("dd", "-", "mmm", "-", "yyyy"), $this->DateFormat, $this->CREATION_DATE->GetValue(true), "", false, $this->ErrorBlock);
        $this->cp["UPDATED_DATE"] = new clsSQLParameter("ctrlUPDATED_DATE", ccsDate, array("dd", "-", "mmm", "-", "yyyy"), $this->DateFormat, $this->UPDATED_DATE->GetValue(true), "", false, $this->ErrorBlock);
        $this->cp["p_area_id"] = new clsSQLParameter("urlp_area_id", ccsFloat, "", "", CCGetFromGet("p_area_id", NULL), 0, false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["UPDATED_DATE"]->GetValue()) and !strlen($this->cp["UPDATED_DATE"]->GetText()) and !is_bool($this->cp["UPDATED_DATE"]->GetValue())) 
            $this->cp["UPDATED_DATE"]->SetValue($this->UPDATED_DATE->GetValue(true));
        if (!is_null($this->cp["p_area_id"]->GetValue()) and !strlen($this->cp["p_area_id"]->GetText()) and !is_bool($this->cp["p_area_id"]->GetValue())) 
            $this->cp["p_area_id"]->SetText(CCGetFromGet("p_area_id", NULL));
        if (!strlen($this->cp["p_area_id"]->GetText()) and !is_bool($this->cp["p_area_id"]->GetValue(true))) 
            $this->cp["p_area_id"]->SetText(0);
        $this->SQL = "UPDATE ifp.p_area \n" .
        "SET \n" .
...
//Delete Method @61-750784B0
    function Delete()
    {
        ...
        if (!strlen($this->cp["p_area_id"]->GetText()) and !is_bool($this->cp["p_area_id"]->GetValue(true))) 
            $this->cp["p_area_id"]->SetText(0);
        $this->SQL = "DELETE FROM ifp.p_area WHERE  p_area_id = " . $this->SQLValue($this->cp["p_area_id"]->GetDBValue(), ccsFloat) . "";
...
//Include events file @1-EEFE3C58
include_once("./P_AREA_events.php");
