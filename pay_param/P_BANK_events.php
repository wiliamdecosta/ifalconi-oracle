...
//End GRID_GRID_Insert_BeforeShow
  ...
  // -------------------------
  	...
  	$GRID->GRID_Insert->Parameters = CCRemoveParam($GRID->GRID_Insert->Parameters, "p_bank_id");
...
//End GRID_BeforeShowRow
  ...
  // -------------------------
      // Write your own code here.
  	$keyId = CCGetFromGet("p_bank_id", "");
  	$sCode = CCGetFromGet("s_keyword", "");
  	global $id;
  	if (empty($keyId)) {
  		if (empty($id)) {
  			$id = $GRID->p_bank_id->GetValue();
  		}
  		global $FileName;
  		global $PathToCurrentPage;
  		$param = CCGetQueryString("QueryString", "");
  		$param = CCAddParam($param, "p_bank_id", $id);
  	...
  	if ($GRID->p_bank_id->GetValue() == $keyId) {
...
//Page_OnInitializeView @1-3D7C9399
function Page_OnInitializeView(& $sender)
{
    $Page_OnInitializeView = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $P_BANK; //Compatibility
