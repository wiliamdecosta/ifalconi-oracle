...
//End GRID_GRID_Insert_BeforeShow
  ...
  // -------------------------
  	...
  	$GRID->GRID_Insert->Parameters = CCRemoveParam($GRID->GRID_Insert->Parameters, "p_area_id");
...
//End GRID_BeforeShowRow
  ...
  // -------------------------
      // Write your own code here.
  	$keyId = CCGetFromGet("p_area_id", "");
  	$sCode = CCGetFromGet("s_keyword", "");
  	global $id;
  	if (empty($keyId)) {
  		if (empty($id)) {
  			$id = $GRID->p_area_id->GetValue();
  		}
  		global $FileName;
  		global $PathToCurrentPage;
  		$param = CCGetQueryString("QueryString", "");
  		$param = CCAddParam($param, "p_area_id", $id);
  	...
  	if ($GRID->p_area_id->GetValue() == $keyId) {
...
//Page_OnInitializeView @1-ED3ACFC7
function Page_OnInitializeView(& $sender)
{
    $Page_OnInitializeView = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $P_AREA; //Compatibility
